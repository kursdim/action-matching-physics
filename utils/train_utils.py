...
class DataGenerator:
    def __init__(self, psi, config):
        ...
        self.samples, self.actions, self.times = None, None, None
    ...
    def gen_data(self):
        ...
        actions = torch.zeros([n_steps, batch_size, psi.dim], device=torch.device('cpu')) ############
        samples[0] = dynamics.samples.cpu()
        times = torch.zeros(n_steps + 1, device=torch.device('cpu'))
        for i in range(n_steps):
            dynamics.propagate(dt)
            actions[i] = dynamics.action.cpu() ##########
            samples[i+1] = dynamics.samples.cpu()
            times[i+1] = times[i] + dt
        self.samples, self.actions, self.times = samples, actions, times
